...
class SchedulerTests(unittest.TestCase):
    ...
    def test_time_range(self):
        with mock_datetime(2014, 6, 28, 12, 0):
            ...
            self.assertGreater(len(minutes), 1)
            self.assertGreaterEqual(min(minutes), 5)
            self.assertLessEqual(max(minutes), 30)
    ...
    def test_at_time(self):
        mock_job = make_mock_job()
        self.assertEqual(every().day.at('10:30').do(mock_job).next_run.hour, 10)
    ...
    def test_at_time_hour(self):
        with mock_datetime(2010, 1, 6, 12, 20):
            mock_job = make_mock_job()
            self.assertEqual(every().hour.at(':30').do(mock_job).next_run.hour, 12)
            self.assertEqual(every().hour.at(':30').do(mock_job).next_run.minute, 30)
            self.assertEqual(every().hour.at(':30').do(mock_job).next_run.second, 0)
            self.assertEqual(every().hour.at(':10').do(mock_job).next_run.hour, 13)
            self.assertEqual(every().hour.at(':10').do(mock_job).next_run.minute, 10)
            self.assertEqual(every().hour.at(':10').do(mock_job).next_run.second, 0)
            self.assertEqual(every().hour.at(':00').do(mock_job).next_run.hour, 13)
    ...
    def test_at_time_minute(self):
        with mock_datetime(2010, 1, 6, 12, 20, 30):
            mock_job = make_mock_job()
            self.assertEqual(every().minute.at(':40').do(mock_job).next_run.hour, 12)
            self.assertEqual(every().minute.at(':40').do(mock_job).next_run.minute, 20)
            self.assertEqual(every().minute.at(':40').do(mock_job).next_run.second, 40)
            self.assertEqual(every().minute.at(':10').do(mock_job).next_run.hour, 12)
    ...
    def test_next_run_time(self):
        with mock_datetime(2010, 1, 6, 12, 15):
            ...
            self.assertEqual(every().minute.do(mock_job).next_run.minute, 16)
            self.assertEqual(every(5).minutes.do(mock_job).next_run.minute, 20)
            self.assertEqual(every().hour.do(mock_job).next_run.hour, 13)
            self.assertEqual(every().day.do(mock_job).next_run.day, 7)
            self.assertEqual(every().day.at('09:00').do(mock_job).next_run.day, 7)
            self.assertEqual(every().day.at('12:30').do(mock_job).next_run.day, 6)
            self.assertEqual(every().week.do(mock_job).next_run.day, 13)
            self.assertEqual(every().monday.do(mock_job).next_run.day, 11)
            self.assertEqual(every().tuesday.do(mock_job).next_run.day, 12)
            self.assertEqual(every().wednesday.do(mock_job).next_run.day, 13)
            self.assertEqual(every().thursday.do(mock_job).next_run.day, 7)
            self.assertEqual(every().friday.do(mock_job).next_run.day, 8)
            self.assertEqual(every().saturday.do(mock_job).next_run.day, 9)
            self.assertEqual(every().sunday.do(mock_job).next_run.day, 10)
    ...
    def test_to_string(self):
        ...
        self.assertIn('job_fun', s)
        self.assertIn('foo', s)
        self.assertIn('bar=23', s)

    def test_to_string_lambda_job_func(self):
        self.assertGreater(len(str(every().minute.do(lambda: 1))), 1)
        self.assertGreater(len(str(every().day.at('10:30').do(lambda: 1))), 1)
    ...
    def test_to_string_functools_partial_job_func(self):
        ...
        self.assertIn('functools.partial', job_repr)
        self.assertIn('bar=True', job_repr)
        self.assertIn('somekey=23', job_repr)
    ...
    def test_run_pending(self):
        ...
        with mock_datetime(2010, 1, 10, 13, 16):
            mock_job.reset_mock()
            schedule.run_pending()
            self.assertEqual(mock_job.call_count, 4)
    ...
    def test_next_run_property(self):
        ...
        with mock_datetime(2010, 1, 6, 13, 16):
            ...
            self.assertEqual(schedule.next_run(), original_datetime(2010, 1, 6, 14, 16))
    ...
    def test_cancel_job(self):
        ...
        self.assertEqual(schedule.jobs[0], mj)
    ...
    def test_tag_type_enforcement(self):
        ...
        job1.tag(0, 'a', True)
        self.assertEqual(len(job1.tags), 3)
