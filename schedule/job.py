...
class Job(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def __str__(self):
        return ("Job(interval={}, " "unit={}, " "do={}, " "args={}, " "kwargs={})").format(self.interval, self.unit,
            self.job_func.__name__, self.job_func.args, self.job_func.keywords, )
    ...
    def __repr__(self):
        ...
        if self.at_time is not None:
            return "Every %s %s at %s do %s %s" % (
                self.interval, self.unit[:-1] if self.interval == 1 else self.unit, self.at_time, call_repr, timestats,)
        else:
            fmt = ("Every %(interval)s " + (
                "to %(latest)s " if self.latest is not None else "") + "%(unit)s do %(call_repr)s %(timestats)s")

            return fmt % dict(interval=self.interval, latest=self.latest,
    ...
    def _schedule_next_run(self):
        ...
        if self.at_time is not None:
            ...
            if not self.last_run:
                ...
                elif (self.unit == "hours" and self.at_time.minute > now.minute or (
                        self.at_time.minute == now.minute and self.at_time.second > now.second)):
